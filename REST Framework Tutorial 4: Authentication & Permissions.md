...
現在我們需要一些用戶，我們最好把用戶呈現型也增加到API上，創建一個新的serializer很容易，在`serializers.py`加入以下程式碼：
...
```
因為 'snippets' 和用戶model是反向關聯（reverse relationship，即多對一），所以在使用`ModelSerializer`時並不會預設加入，所以我們需要加入一顯式字段的來實現。

我們還需要加一些views到views.py，對用戶呈現型上，我們偏好使用唯讀式的view，所以將使用 `ListAPIView` 和 `RetrieveAPIView` 泛型類Views。
...
現在，如果我們創建一個code snippet，還沒有方法可關連到其創建者。User並沒有作為序列化呈現型的一部分發送，而是作為request的一個屬性。
...
現在snippets已經和創建者關聯起來了，我們接下來還需要更新`SnippetSerializer`，在`serizlizers.py`中增加一個新的字段：

    owner = serializers.ReadOnlyField(source='owner.username')

Note: 確定你有在class Meta:的字段列表中也加入了 'owner'。

這個字段所做的十分有趣。source 參數控制某屬性被增加一個新字段，可以指向序列化實例的任何屬性。它可以採用如上的點式表示（dotted notation），
在這個案例它將直接遍歷到指定的屬性。在Django's template中使用時，也可以採用類似的方式。

我們增加字段是一個無類型 `ReadOnlyField` 類，而我們之前的字段都是有類型的，例如 `CharField`, `BooleanField` etc... 
無類型`ReadOnlyField`字段總是只讀的，它們只用在序列化呈現型中，而在反序列化時（修改model）不被使用。在此(反序列化)我們也可以使用`CharField(read_only=True)`
...
我們已經有了一系列的權限，如果我們需要編輯任何snippet，我們需要認證我們的request。因為我們還沒有建立任何 `authentication classes`,
所以目前是默認的SessionAuthentication 和BasicAuthentication在起作用。
...
## 心得
這一小節的主要重點是權限與認證使用者，當然有時候權限會很複雜，認證其實也不容易，REST framework其實都不碰這兩個，這些是使用django 的預設功能。